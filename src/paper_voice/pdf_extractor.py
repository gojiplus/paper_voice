...
class PDFExtractor:
    ...
    def __init__(self):
        """Initialize the extractor and detect available PDF backends."""
    ...
    def extract_document(
        self, pdf_path: str, method: str | None = None
    ) -> ExtractedDocument:
        ...
        Raises:
            RuntimeError: If no usable extraction backend is available,
                either because none is installed or because ``method``
                names a backend that is not.
        """
        if method is None:
        ...
        if method == "pypdf2" and PdfReader is not None:
            return self._extract_with_pypdf2(pdf_path)
        raise RuntimeError(
            f"No suitable PDF extraction method available. "
    ...
    def _extract_with_pymupdf(self, pdf_path: str) -> ExtractedDocument:
        ...
        if fitz is None:  # pragma: no cover - guarded by extract_document
        ...
        # PyMuPDF's bundled stubs omit Document.__iter__, but a Document does
        # iterate over its pages at runtime.
        for page_num, page in enumerate(doc):  # pyright: ignore[reportArgumentType]
    ...
    def _extract_with_pypdf2(self, pdf_path: str) -> ExtractedDocument:
        """Extract using pypdf (simpler, more reliable for text)."""
        if PdfReader is None:  # pragma: no cover - guarded by extract_document
        ...
        # Detect figure references (since we can't extract images with pypdf)
    ...
    def _convert_text_to_latex_markdown(self, text: str) -> str:
        ...
        math_patterns = [
            ...
            # Greek letters (if they appear as Unicode); the Greek chars
            # are intentional math content mapped to speech
            (
                r"\\b(α|β|γ|δ|ε|ζ|η|θ|ι|κ|λ|μ"  # noqa: RUF001
                r"|ν|ξ|π|ρ|σ|τ|υ|φ|χ|ψ|ω)\\b",  # noqa: RUF001
                r"$\\\\alpha$",
            ),  # Example for alpha
        ...
        return re.sub(r"Table\\s+(\\d+)", r"Table~\\\\ref{tab:\\1}", text)
    ...
    def _improve_document_structure(self, text: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if (
                i > 0
                and improved_lines
                and improved_lines[-1]
                and not line[0].isupper()
                and not line.startswith("\\section")
                and not re.match(r"^\\d+\\.", line)
                and len(improved_lines[-1]) > 40
            ):  # Previous line is substantial
    ...
    def _associate_captions_with_objects(
        self, text: str, figures: list[dict[str, str]], tables: list[dict[str, str]]
    ) -> None:
        """Associate captions with figures and tables."""
...
def extract_pdf_to_latex(pdf_path: str, method: str | None = None) -> ExtractedDocument:
