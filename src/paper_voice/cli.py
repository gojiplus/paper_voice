...
from .vision_pdf_analyzer import (
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  %(prog)s https://arxiv.org/abs/2301.12345 --api-key YOUR_KEY"
            " --output research.mp3\n"
            "  %(prog)s paper.tex --latex --api-key YOUR_KEY --voice nova\n"
            "  %(prog)s paper.pdf --vision --api-key YOUR_KEY --output enhanced.mp3\n"
            "  %(prog)s --batch papers/ --api-key YOUR_KEY --output-dir"
            " ./audio_output\n"
            "  %(prog)s --batch paper1.pdf paper2.pdf paper3.tex --api-key YOUR_KEY"
    ...
    parser.add_argument(
        "input",
        nargs="*",
        help="Input file path(s), directory, arXiv URL(s)/ID(s), or text content."
    ...
    parser.add_argument(
        ...
        help="Output audio file path (default: paper_voice_output.mp3)."
        " For batch mode, use --output-dir",
    )
    ...
    input_group.add_argument(
        "--pdf", action="store_true", help="Treat input as PDF file"
    )
    input_group.add_argument(
        "--latex", action="store_true", help="Treat input as LaTeX file"
    )
    input_group.add_argument(
        "--arxiv", action="store_true", help="Treat input as arXiv URL or ID"
    )
    input_group.add_argument(
        "--text",
        action="store_true",
    ...
    parser.add_argument(
        "--api-key", required=True, help="OpenAI API key for LLM-powered enhancements"
    )
...
def collect_input_files(inputs: list[str], _args: argparse.Namespace) -> list[str]:
    """Collect all input files from the provided inputs (files, directories, URLs)."""
    all_inputs = []

    for input_item in inputs:
        if Path(input_item).is_dir():
            # Directory: find all supported files
            patterns = ["*.pdf", "*.tex", "*.latex", "*.txt", "*.md"]
            for pattern in patterns:
                found_files = [str(p) for p in Path(input_item).glob(pattern)]
                all_inputs.extend(found_files)
            # Also check subdirectories
            for pattern in patterns:
                found_files = [str(p) for p in Path(input_item).rglob(pattern)]
                all_inputs.extend(found_files)
        elif Path(input_item).is_file():
            # Single file
...
def determine_input_type(input_path: str, args: argparse.Namespace) -> str:
    ...
    if input_path.startswith("http") and "arxiv.org" in input_path:
        return "arxiv"
    if input_path.isdigit() or ("." in input_path and len(input_path.split(".")) == 2):
...
def load_content(
    input_path: str, input_type: str, args: argparse.Namespace
) -> tuple[str, str]:
    ...
    if input_type == "pdf":
        ...
        if args.vision:
            ...
            if args.verbose:
                print(
                    "Vision analysis: "
                    f"{len(analysis_result.content_blocks)} content blocks"
                )
        else:
...
def generate_output_path(
    input_path: str, args: argparse.Namespace, batch_mode: bool = False
) -> str:
...
def process_batch(inputs: list[str], args: argparse.Namespace) -> None:
    ...
    if args.verbose:
        print(
            f"Batch processing {len(inputs)} inputs with {args.max_workers} workers..."
        )
    ...
    with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        ...
        future_to_input = {
            executor.submit(
                process_single_paper, input_path, args, batch_mode=True
            ): input_path
...
def generate_audio(content: str, args: argparse.Namespace) -> str:
    ...
    try:
        ...
        return tts.synthesize_speech_chunked(
            content,
            args.output,
