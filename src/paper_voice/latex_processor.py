...
def _handle_fraction(match) -> str:
    r"""Handle \frac{numerator}{denominator}."""
...
def _handle_sqrt(match) -> str:
    r"""Handle \sqrt[n]{content} or \sqrt{content}."""
...
# Matches \sum, \prod, \int with optional lower/upper limits, braced
# ({i=1}) or not (_0, ^\infty).
_BIG_OPERATOR_RE = re.compile(
    r"\\(sum|prod|int)(?![a-zA-Z])"
    r"(?:_(\{[^{}]*\}|\\[a-zA-Z]+|[^\s{\\^]))?"
    r"(?:\^(\{[^{}]*\}|\\[a-zA-Z]+|[^\s{\\]))?"
)

_BIG_OPERATOR_NAMES = {"sum": "sum", "prod": "product", "int": "integral"}


def _handle_sum_prod_int(match: re.Match[str]) -> str:
    r"""Handle \sum, \prod, \int with optional limits."""
    command = match.group(1)
    lower = match.group(2)
    upper = match.group(3)

    result = _BIG_OPERATOR_NAMES.get(command, command)
    if lower:
        result += f" from {latex_math_to_speech(lower.strip('{}'))}"
    if upper:
        result += f" to {latex_math_to_speech(upper.strip('{}'))}"
...
def latex_math_to_speech(expr: str) -> str:
    ...
    while i < len(tokens):
        ...
        if token.startswith("\\") and token in MATH_OPERATORS:
        ...
        elif token == "^" and i + 1 < len(tokens):  # noqa: S105 - math token
            output.append(_parse_superscript(tokens[i + 1]))
            i += 1
        elif token == "_" and i + 1 < len(tokens):  # noqa: S105 - math token
            ...
            output.append(bracket_map[token])
        elif token in ["+", "-", "=", "<", ">", "/", "*", ",", "|", "!"]:
            symbol_map = {
                "+": "plus",
                "-": "minus",
...
def _tokenize_latex(expr: str) -> list[str]:
    ...
    while i < len(expr):
        if expr[i] == "\\":
            ...
            # Collect alphanumeric sequences ("_" stays a separate token: in
            # LaTeX math it always means subscript, never an identifier part)
            j = i
...
def process_latex_environments(
    text: str, environments: dict[str, list[tuple[int, int, str]]]
) -> str:
...
def process_inline_and_display_math(
    text: str, api_key: str | None = None, use_llm: bool = True
) -> str:
    ...
    math_patterns = [
        ...
        (r"\$\$(.*?)\$\$", "display", re.DOTALL),  # $$...$$
        (r"\\\[(.*?)\\\]", "display", re.DOTALL),  # \[...\]
        # Inline math patterns: $...$ but not $$...$$ and not escaped \$
        (r"(?<!\$)(?<!\\)\$([^$]+?)(?<!\\)\$(?!\$)", "inline", 0),
        (r"\\\((.*?)\\\)", "inline", 0),  # \(...\)
    ]
    ...
    if use_llm and api_key:
        ...
        try:
            ...
            return enhance_document_simple(text, api_key)
...
def process_latex_document(
    ...
    if abstract_match:
        metadata["abstract"] = clean_latex_commands(abstract_match.group(1))
...
def process_markdown_with_math(
    text: str, api_key: str | None = None, use_llm: bool = True
) -> str:
    """Process Markdown text with LaTeX math expressions."""
    # Process LaTeX math in markdown
    return process_inline_and_display_math(text, api_key, use_llm)
