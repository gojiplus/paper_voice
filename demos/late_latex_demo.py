#!/usr/bin/env python3
"""Demo of processing the LATE LaTeX document.

Converts the document's mathematical content to spoken text using simple
regex-based LaTeX-to-speech conversion, without requiring the full package.
"""
...
This document tests our enhanced LaTeX processing capabilities with
mathematical content related to Local Average Treatment Effects (LATE).

\section{Mathematical Framework}

Let $Y_i(d)$ denote the potential outcome for individual $i$ under treatment
status $d \in \{0,1\}$. The observed outcome is:
...
In the context of cross-fitting, we partition the sample into $K$ folds. For
each fold $k$, we estimate nuisance parameters $\hat{\eta}^{(-k)}$ using the
complement sample. The cross-fitted estimator is:
...
where $\omega_i$ are random weights satisfying $E[\omega_i] = 1$ and
$\text{Var}(\omega_i) = \sigma^2$.
...
If $\mathcal{F}$ is a Donsker class and
$\sup_{f \in \mathcal{F}} |P_n f - P f| = o_p(n^{-1/2})$, then:
$$\sup_{x \in \mathbb{R}} |P^*(\sqrt{n} \sup_{f \in \mathcal{F}}
|\mathbb{G}_n^*(f)| \leq x) - P(\sqrt{n} \sup_{f \in \mathcal{F}}
|\mathbb{G}_n(f)| \leq x)| \xrightarrow{p} 0$$
\end{proposition}

\section{Application to LATE Estimation}

The LATE estimand can be written as:
$$\beta_{LATE} = \frac{E[Y_i | Z_i = 1] - E[Y_i | Z_i = 0]}
...
$$\hat{\beta}_{LATE} =
\frac{\frac{1}{n}\sum_{i=1}^{n} (\hat{\mu}_{11}^{(-k(i))} - \hat{\mu}_{01}^{(-k(i))})}
{\frac{1}{n}\sum_{i=1}^{n} (\hat{\pi}_{11}^{(-k(i))} - \hat{\pi}_{01}^{(-k(i))})}$$

\section{Conclusion}

This framework demonstrates the application of cross-fitting to LATE
estimation, ensuring robust inference even when using flexible machine
...
def main():
    """Run the LaTeX processing demo on the LATE document."""
    print("=" * 80)
    ...
    try:
        ...
        if len(paragraphs) > shown_paragraphs:
            print(
                f"... [showing first {shown_paragraphs} paragraphs "
        ...
        math_examples = [
            ...
            r"$$\hat{\theta} = \frac{1}{n} \sum_{i=1}^{n} "
            r"\psi(W_i; \hat{\eta}^{(-k(i))})$$",
        ...
        # Greek letters are intentional math content, not typos for Latin.
        print("  • Greek letters (α → 'alpha', Σ → 'big sigma')")  # noqa: RUF001
