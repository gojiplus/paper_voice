...
def show_math_improvement_examples():
    ...
    print(
        '  "square root of n (hat theta minus theta sub 0) '
        'converges to d N(0, big sigma)"'
    )
    print()
    ...
    examples = [
        {
            "latex": "\\hat{\\theta} = \\frac{1}{n} \\sum_{i=1}^{n} "
            "\\psi(W_i; \\hat{\\eta}^{(-k(i))})",
            "bad": "hat theta equals fraction one over n sum from i equals one "
            "to n psi W sub i semicolon hat eta superscript minus k of i",
            "good": "Theta hat, which represents our estimator, is calculated "
            "as the average over all n observations. Specifically, we take the "
            "sum from i equals 1 to n of the function psi, evaluated at W "
            "subscript i, using the auxiliary parameter eta hat that was "
            "estimated on the complement sample excluding fold k of i, then "
            "divide this sum by n.",
        },
        {
            "latex": "\\sqrt{n}(\\hat{\\theta} - \\theta_0) "
            "\\xrightarrow{d} N(0, \\Sigma)",
            "bad": "square root of n open parenthesis hat theta minus theta "
            "sub 0 close parenthesis converges to d N open parenthesis 0 comma "
            "big sigma close parenthesis",
            "good": "As the sample size grows large, the quantity square root "
            "of n times the difference between our estimator theta hat and the "
            "true parameter theta naught converges in distribution to a normal "
            "distribution with mean zero and covariance matrix capital Sigma. "
            "This is a fundamental result showing that our estimator is "
            "asymptotically normal.",
        },
        {
            "latex": "E[Y_i(1) - Y_i(0) | \\text{complier}]",
            "bad": "E open bracket Y sub i open parenthesis 1 close parenthesis "
            "minus Y sub i open parenthesis 0 close parenthesis pipe complier "
            "close bracket",
            "good": "The expected value of the treatment effect for individual "
            "i, given that the individual is a complier. This represents the "
            "difference between their potential outcome under treatment Y "
            "subscript i of 1 and their potential outcome under control Y "
            "subscript i of 0, conditional on being a complier type.",
        },
        {
            "latex": "\\beta_{LATE} = \\frac{E[Y_i | Z_i = 1] - "
            "E[Y_i | Z_i = 0]}{E[D_i | Z_i = 1] - E[D_i | Z_i = 0]}",
            "bad": "beta sub LATE equals fraction E bracket Y sub i pipe Z sub "
            "i equals 1 close bracket minus E bracket Y sub i pipe Z sub i "
            "equals 0 close bracket over E bracket D sub i pipe Z sub i equals "
            "1 close bracket minus E bracket D sub i pipe Z sub i equals 0 "
            "close bracket",
            "good": "The Local Average Treatment Effect, beta LATE, is "
            "calculated as the ratio of two quantities. The numerator is the "
            "difference in expected outcomes: the expected value of Y for "
            "individual i when the instrument Z subscript i equals 1, minus "
            "the expected value when the instrument equals 0. The denominator "
            "is the corresponding difference in treatment probabilities: the "
            "expected treatment rate when the instrument equals 1, minus the "
            "expected treatment rate when the instrument equals 0.",
        },
    ]
...
def show_technical_approach():
    ...
    prompt_excerpt = """You are a world-class mathematics exposition expert.
Your job is to convert mathematical expressions into crystal-clear natural
language that a listener can easily understand through audio narration.

CRITICAL REQUIREMENTS:
1. Use PRECISE language that distinguishes between variables
   (e.g., "capital X" vs "lowercase x")
