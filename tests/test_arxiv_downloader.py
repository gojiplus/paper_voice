...
class TestTexFileDetection:
    ...
    def test_find_main_tex_file_single(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert result == str(tex_file)

    def test_find_main_tex_file_with_documentclass(self):
        """Test finding file with documentclass."""
        with tempfile.TemporaryDirectory() as temp_dir:
            # Create multiple tex files
            (Path(temp_dir) / "section1.tex").write_text("\\section{Introduction}")

            main_file = Path(temp_dir) / "main.tex"
            main_file.write_text("\\documentclass{article}\\begin{document}")
...
class TestLatexInputProcessing:
    ...
    def test_process_latex_inputs_simple(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (Path(temp_dir) / "section1.tex").write_text("This is section 1 content.")
    ...
    def test_process_latex_inputs_with_extension(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            (Path(temp_dir) / "section1.tex").write_text("Section content")
...
class TestArxivDownload:
    ...
    def test_download_arxiv_paper_success(self, mock_tarfile, mock_requests):
        ...
        with (
            patch("paper_voice.arxiv_downloader.find_main_tex_file") as mock_find_tex,
            patch(
                "paper_voice.arxiv_downloader.extract_figures_from_source"
            ) as mock_figures,
            patch(
                "paper_voice.arxiv_downloader.Path.read_text",
                return_value="\\title{Test Paper}",
            ),
        ):
            # Setup mocks
            mock_find_tex.return_value = "/nonexistent/main.tex"
