...
class TestChunkedSynthesis:
    ...
    def test_synthesize_short_text_no_chunking(self, mock_audio, mock_synth):
        ...
        output_path = "/nonexistent/test.mp3"
    ...
    @patch("shutil.rmtree")
    def test_synthesize_long_text_with_chunking(
        self, mock_rmtree, mock_tempdir, mock_audio, mock_synth
    ):
        ...
        # Mock tempfile
        mock_tempdir.return_value = "/nonexistent/chunks"

        # Mock synthesize_speech calls
        mock_synth.side_effect = [
            "/nonexistent/chunks/chunk_000.mp3",
            "/nonexistent/chunks/chunk_001.mp3",
        ]
        ...
        mock_rmtree.assert_called_once_with("/nonexistent/chunks", ignore_errors=True)
    ...
    def test_synthesize_offline_no_chunking_needed(self, mock_synth):
        ...
        output_path = "/nonexistent/test.wav"

        mock_synth.return_value = output_path

        result = synthesize_speech_chunked(text, output_path, use_openai=False)
    ...
    def test_chunking_error_handling(self, mock_audio, mock_synth):
        ...
        with (
            patch("paper_voice.tts.AudioSegment", None),
            pytest.raises(RuntimeError, match="pydub is required"),
        ):
            synthesize_speech_chunked(
                long_text, output_path, use_openai=True, api_key="test-key"
            )
...
class TestIntegration:
    ...
    def test_speech_synthesis_parameters(self):
        ...
        with patch("paper_voice.tts.synthesize_speech") as mock_synth:
            mock_synth.return_value = "/nonexistent/test.mp3"
