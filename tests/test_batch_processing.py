...
class TestBatchProcessing:
    ...
    def setup_method(self):
        ...
        # Keep generated output inside the temp dir: generate_output_path
        # really creates output_dir, and "./output" would pollute the repo.
    ...
    def test_collect_input_files_directory(self):
        ...
        for filename in test_files:
            (Path(self.temp_dir) / filename).write_text("dummy content")
        ...
        assert len(result) >= 4
        found_files = [Path(f).name for f in result]
    ...
    def test_collect_input_files_mixed(self):
        ...
        # Create test file
        test_file = Path(self.temp_dir) / "paper.pdf"
        test_file.write_text("dummy content")

        inputs = [str(test_file), "https://arxiv.org/abs/2301.12345", "2405.67890"]
        result = collect_input_files(inputs, self.args)
    ...
    def test_collect_input_files_duplicates(self):
        ...
        inputs = [str(test_file), str(test_file), str(test_file)]
        result = collect_input_files(inputs, self.args)
    ...
    def test_generate_output_path_batch_mode_file(self):
        ...
        test_file = Path(self.temp_dir) / "research_paper.pdf"
        test_file.write_text("dummy")

        result = generate_output_path(str(test_file), self.args, batch_mode=True)

        assert result.endswith("research_paper.mp3")
        assert Path(result).parent == Path(self.args.output_dir)
    ...
    @patch("os.makedirs")
    def test_process_single_paper_success(
        self,
        mock_makedirs,
        mock_generate_audio,
        mock_process_math,
    ...
    def test_process_batch(
        self, mock_makedirs, mock_process_single, mock_executor_class
    ):
        ...
        mock_executor.__enter__.return_value.as_completed = Mock(
            return_value=[mock_future1, mock_future2]
        )

        # Mock as_completed
        with patch(
            "paper_voice.cli.as_completed", return_value=[mock_future1, mock_future2]
        ):
            ...
            # The batch summary should report both papers as successful
            printed = "\n".join(
...
class TestBatchIntegration:
    ...
    def test_end_to_end_batch_workflow(self):
        ...
        from paper_voice.cli import setup_parser
