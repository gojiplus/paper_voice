...
class TestFigureTableExtraction:
    ...
    def test_extract_figures_with_captions(self):
        ...
        analysis_result = PDFAnalysisResult(
            content_blocks=[
                ContentBlock(
                    "figure", "", "Bar chart showing results", (10, 20, 90, 60), 1
                ),
                ContentBlock(
                    "caption", "Figure 1: Results comparison", "", (10, 65, 90, 75), 1
                ),
            ],
            page_images=[b"page"],
            metadata={},
    ...
    def test_extract_tables_with_captions(self):
        ...
        analysis_result = PDFAnalysisResult(
            content_blocks=[
                ContentBlock(
                    "caption", "Table 1: Statistical summary", "", (10, 10, 90, 20), 1
                ),
...
class TestPDFToImages:
    ...
    def test_pdf_to_images_success(self, mock_fitz):
        ...
        # Mock PyMuPDF objects (MagicMock: the code indexes doc[page_num])
        mock_doc = MagicMock()
        mock_page = Mock()
        mock_pix = Mock()
    ...
    def test_pdf_to_images_no_pymupdf(self):
        ...
        with (
            patch("paper_voice.vision_pdf_analyzer.PYMUPDF_AVAILABLE", False),
            pytest.raises(RuntimeError, match="PyMuPDF is required"),
        ):
