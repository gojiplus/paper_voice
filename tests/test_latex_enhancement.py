...
class TestLatexEnhancement:
    ...
    def test_latex_math_with_llm(self):
        ...
        The main result shows that $\alpha^2 + \beta = \gamma$ where alpha
        ...
        enhanced_result = """
        The main result shows that alpha squared plus beta equals gamma
        where alpha is the learning rate.

        We also have the display equation: the integral from 0 to infinity
        of f of x dx equals the sum from n equals 1 to infinity of 1 over
        n squared.
        ...
        with patch(
            ...
            result = process_latex_content(
                latex_content, api_key="fake-key", use_llm_enhancement=True
            )
    ...
    def test_latex_figures_with_llm(self):
        ...
        \caption{Network architecture showing input layer, hidden layers,
        and output layer.}
        \end{figure}
        ...
        enhanced_caption = (
            "A comprehensive diagram illustrating the neural network structure "
            "with clearly defined input processing, intermediate feature "
            "extraction layers, and final output generation."
        )

        with patch(
            "paper_voice.figure_table_summarizer.summarize_figure_with_llm",
            return_value=enhanced_caption,
        ):
            processed = process_latex_document(
                latex_content, summarize_figures=True, api_key="fake-key"
            )
    ...
    def test_latex_tables_with_llm(self):
        ...
        enhanced_caption = (
            "Comprehensive performance comparison table presenting accuracy "
            "percentages and processing speed characteristics for two distinct "
            "machine learning models, enabling direct comparison of their "
            "effectiveness and efficiency trade-offs."
        )

        with patch(
            "paper_voice.figure_table_summarizer.summarize_table_with_llm",
            return_value=enhanced_caption,
        ):
            processed = process_latex_document(
                latex_content, summarize_tables=True, api_key="fake-key"
            )
    ...
    def test_unified_processor_latex(self):
        ...
        This paper presents energy equals mass times the speed of light squared
        which is Einstein's famous equation.

        Figure: Detailed visualization showing the fundamental relationship
        between energy and mass as described by Einstein's theory of special
        relativity.
        ...
        with (
            patch(
                "paper_voice.simple_llm_enhancer.enhance_document_simple",
                return_value=enhanced_content,
            ),
            patch(
                "paper_voice.figure_table_summarizer.summarize_figure_with_llm",
                return_value="Enhanced figure description",
            ),
        ):
            ...
            assert (
                ...
                in processed_doc.enhanced_text
            )

            # Check content characteristics
            assert processed_doc.has_math or processed_doc.has_figures
    ...
    def test_latex_without_llm_enhancement(self):
        ...
        assert (
            "x squared plus y squared equals z squared" in processed_doc.enhanced_text
        )
    ...
    def test_preserve_non_math_text_in_latex(self):
        ...
        This research paper investigates the computational complexity of
        machine learning algorithms in distributed systems. We propose a novel
        framework that optimizes resource allocation while maintaining high
        accuracy levels.

        The mathematical foundation relies on $\theta = \arg\min L(\theta)$
        optimization.

        Our experimental evaluation demonstrates significant improvements over
        existing approaches. The proposed method achieves 95% accuracy while
        reducing computational overhead by 40%. These results indicate
        ...
        enhanced_content = """
        This research paper investigates the computational complexity
        of machine learning algorithms in distributed systems.
        We propose a novel framework that optimizes resource allocation
        while maintaining high accuracy levels.

        The mathematical foundation relies on
        theta equals the argument that minimizes the loss function of theta
        optimization.

        Our experimental evaluation demonstrates significant improvements
        over existing approaches.
        The proposed method achieves 95% accuracy while reducing
        computational overhead by 40%.
        These results indicate practical advantages
...
class TestLatexIntegration:
    ...
    def test_complex_latex_document(self):
        ...
        Section: Methodology
        The core algorithm minimizes the cost function J of theta equals
        one over two m times the sum from i equals 1 to m of the squared
        difference between the hypothesis function and the actual value.

        Figure: Detailed graph showing the convergence behavior of the
        optimization algorithm over iterations.

        Table: Comprehensive comparison of various optimization techniques.

        Section: Results
        The algorithm converges to theta star equals the argument that minimizes
        the cost function J of theta efficiently.

        Section: Conclusion
        Our method demonstrates superior performance across all metrics.
        """

        enhanced_fig_caption = (
            "Detailed graph showing the convergence behavior of the optimization "
            "algorithm over iterations, including convergence rate and final "
            "accuracy achieved."
        )
        enhanced_table_caption = (
            "Comprehensive comparison table presenting performance metrics "
            "including convergence speed, final accuracy, and computational "
            "requirements for various optimization techniques."
        )
        ...
        with (
            ...
            patch(
                "paper_voice.figure_table_summarizer.summarize_figure_with_llm",
                return_value=enhanced_fig_caption,
            ),
            patch(
                "paper_voice.figure_table_summarizer.summarize_table_with_llm",
                return_value=enhanced_table_caption,
            ),
        ):
            processed_doc = process_content_unified(
